...
export function LocationsPage() {
  ...
  async function importIcon(event: ChangeEvent<HTMLInputElement>) {
    ...
    try { const customIconDataUrl = await uploadDataUrlIfNeeded(await readImage(file)); setForm((value) => ({ ...value, customIconDataUrl, iconId: 'custom' })) } catch (reason) { setError(reason instanceof Error ? reason.message : String(reason)) }
  }
  ...
  return <div className="relative flex h-[var(--app-height)] flex-col overflow-hidden bg-[#f4f4f6]">
    ...
    <main className="relative min-h-0 flex-1">{map ? <>
      ...
      {!movingId && !draftPoint && <div className="pointer-events-none absolute inset-x-14 bottom-5 z-10 rounded-full bg-gray-900/90 px-4 py-2 text-center text-xs text-white">点空白格显示＋，地点周围一格内不可放置</div>}
    ...
    {selected && <div className="absolute inset-x-0 bottom-0 z-30 max-h-[68%] overflow-y-auto rounded-t-3xl bg-white p-4 pb-[calc(env(safe-area-inset-bottom)+16px)] shadow-2xl">

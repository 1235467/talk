...
export function LocationsPage() {
  ...
  return <div className="relative flex h-[var(--app-height)] flex-col overflow-hidden bg-[#f4f4f6]">
    <TopBar title="地点" showBack right={<div className="flex items-center gap-1"><button type="button" title="重新生成地图" aria-label="重新生成地图" disabled={regenerating} onClick={() => void regenerate()} className="flex h-9 w-9 items-center justify-center text-gray-700 disabled:opacity-40"><RotateCcw size={17} className={regenerating ? 'animate-spin' : ''} /></button><button type="button" title={editMode ? '完成编辑' : '编辑地点'} aria-label={editMode ? '完成编辑' : '编辑地点'} onClick={() => { setEditMode((value) => !value); setMovingId(undefined); setFormMode(undefined); setDraftPoint(undefined); setSelectedId(undefined) }} className={`flex h-9 w-9 items-center justify-center rounded-lg ${editMode ? 'bg-[var(--ui-special-soft)] text-[var(--ui-special-ink)]' : 'text-gray-700'}`}><Pencil size={18} /></button></div>} />
    <main className="relative min-h-0 flex-1">{map ? <>
      ...
      <div className="pointer-events-none absolute left-3 top-3 z-10 max-w-[calc(100%-76px)] rounded-xl border border-[var(--ui-border)] bg-[var(--ui-surface)] px-3 py-2 shadow-lg"><p className="truncate text-xs font-medium text-[var(--ui-text)]">{active ? `当前在 ${active.name}` : '选择一个地点进入'}</p><p className="mt-0.5 text-[10px] text-[var(--ui-text-3)]">{timeText} · {realSeason(now)}</p></div>

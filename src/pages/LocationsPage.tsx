...
import { MapPinned, Pencil, Plus, RotateCcw, Upload, X } from 'lucide-react'
...
import { childLocations, enterLocation, realSeason, regenerateLocationMap, syncContactLocationsAt } from '../lib/locations'
...
export function LocationsPage() {
  ...
  async function deleteSelected() {
    if (!selected?.userCreated || !window.confirm(`删除地点“${selected.name}”及其子地点？此操作无法撤销。`)) return
    const childIds = new Set(childLocations(selected.id, locations).map((item) => item.id))
    const deletedIds = new Set([selected.id, ...childIds])
    const affected = contacts.filter((contact) => contact.currentLocationId && deletedIds.has(contact.currentLocationId))
    await db.transaction('rw', db.locations, db.contacts, db.locationModuleState, async () => {
      ...
      if (state?.currentLocationId && deletedIds.has(state.currentLocationId)) await db.locationModuleState.update('active', { currentLocationId: undefined, updatedAt: Date.now() })
    })
  ...
  async function deleteChild(child: LocationNode) {
    if (!child.userCreated || !window.confirm(`删除子地点“${child.name}”？`)) return
    await db.locations.delete(child.id)
    for (const contact of contacts.filter((item) => item.currentLocationId === child.id)) await db.contacts.update(contact.id, { currentLocationId: undefined, locationSource: undefined, locationUpdatedAt: Date.now() })
    if (state?.currentLocationId === child.id) await db.locationModuleState.update('active', { currentLocationId: undefined, updatedAt: Date.now() })
  }

  async function regenerate() {
    if (!window.confirm('重新生成会更换地形并重新分配所有顶层地点，但会保留名称、备注、图标和子地点。确定继续？')) return
    setRegenerating(true); setError('')
    try { await regenerateLocationMap(); setManageOpen(false); setSelectedId(undefined); setDraftPoint(undefined) }
    catch (reason) { setError(reason instanceof Error ? reason.message : String(reason)) }
  ...
  return <div className="relative flex h-[var(--app-height)] flex-col overflow-hidden bg-[#f4f4f6]">
    ...
    <main className="relative min-h-0 flex-1">{map ? <>
      <LocationMapCanvas map={map} locations={locations} activeLocationId={state?.currentLocationId} contacts={contacts} selectedLocationId={selectedId} draftPoint={draftPoint} editing={editMode} placementMode={!!movingId} onBuildingClick={handleMarker} onMapClick={(point) => void handleMapClick(point)} onConfirmDraft={beginNewLocation} />
      ...
      {editMode && !movingId && !draftPoint && <div className="pointer-events-none absolute inset-x-14 bottom-5 z-10 rounded-full bg-gray-900/90 px-4 py-2 text-center text-xs text-white">点空白格显示＋，地点周围一格内不可放置</div>}
    ...
    {selected && !editMode && <div className="absolute inset-x-0 bottom-0 z-30 max-h-[68%] overflow-y-auto rounded-t-3xl bg-white p-4 pb-[calc(env(safe-area-inset-bottom)+16px)] shadow-2xl">
      <div className="flex items-start justify-between"><div><h2 className="font-semibold text-gray-900">{selected.name}</h2><p className="mt-0.5 text-xs text-gray-400">{selected.description}</p>{selected.note && <p className="mt-2 rounded-xl bg-amber-50 px-3 py-2 text-xs text-amber-800">备注：{selected.note}</p>}</div><button type="button" onClick={() => setSelectedId(undefined)} className="flex h-8 w-8 items-center justify-center text-gray-400"><X size={20} /></button></div>
    ...
    {manageOpen && map && <div className="absolute inset-0 z-40 flex flex-col bg-[#f4f4f6]">
      ...
      <div className="flex-1 overflow-y-auto p-4 pb-8">
        <section className="rounded-2xl bg-white p-4"><h2 className="text-sm font-semibold text-gray-900">地点编辑</h2><p className="mt-1 text-xs text-gray-400">在空白格新增地点，或修改现有地点、备注与子地点</p><button type="button" onClick={() => { setEditMode(true); setManageOpen(false); setSelectedId(undefined); setDraftPoint(undefined) }} className="mt-3 w-full rounded-xl bg-gray-900 px-3 py-2.5 text-xs text-white">进入编辑模式</button></section>
        <section className="mt-3 rounded-2xl bg-white p-4"><h2 className="text-sm font-semibold text-gray-900">重新生成地图</h2><p className="mt-1 text-xs leading-5 text-gray-400">重新生成地形并分配全部顶层地点。地点资料、用户备注、图标和子地点都会保留。</p><button type="button" disabled={regenerating} onClick={() => void regenerate()} className="mt-3 flex w-full items-center justify-center gap-2 rounded-xl border border-gray-200 px-3 py-2.5 text-xs text-gray-700 disabled:opacity-50"><RotateCcw size={15} />{regenerating ? '正在重新生成…' : '重新生成并分配地点'}</button></section>
      </div>
    </div>}

    {editMode && <button type="button" onClick={() => { setEditMode(false); setMovingId(undefined); setFormMode(undefined); setDraftPoint(undefined) }} className="absolute right-3 top-[54px] z-30 rounded-full bg-white px-3 py-2 text-xs font-medium text-gray-700 shadow-lg">完成编辑</button>}

    {formMode && <div className="absolute inset-x-0 bottom-0 z-50 max-h-[82%] overflow-y-auto rounded-t-3xl bg-white p-4 pb-[calc(env(safe-area-inset-bottom)+16px)] shadow-2xl">
      <div className="flex items-center justify-between"><h2 className="font-semibold text-gray-900">{formMode === 'new' ? '新增地点' : '编辑地点'}</h2><button type="button" onClick={() => { setFormMode(undefined); setDraftPoint(undefined); setChildForm(undefined) }}><X size={20} /></button></div>
      <label className="mt-4 block text-xs text-gray-500">地点名称<input value={form.name} onChange={(event) => setForm((value) => ({ ...value, name: event.target.value }))} className="mt-1 w-full rounded-xl border px-3 py-2.5 text-sm" placeholder="例如：星河公寓" /></label>

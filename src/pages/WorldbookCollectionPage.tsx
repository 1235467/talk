...
export function WorldbookCollectionPage() {
  ...
  const { data: collection } = useQuery({ queryKey: ['worldbookCollections', collectionId], queryFn: () => getOrUndef(api.worldbookCollections.get(collectionId)) })
  const { data: entries = EMPTY_ENTRIES } = useQuery({ queryKey: ['worldbookEntries', collectionId], queryFn: () => api.worldbookEntries.list({ collectionId }) })
  const { data: libraryItems = EMPTY_LIBRARY_ITEMS } = useQuery({ queryKey: ['libraryItems'], queryFn: () => api.libraryItems.list() })
  const { data: boundContacts = 0 } = useQuery({ queryKey: ['contacts', 'bound-world', collectionId], queryFn: async () => (await api.contacts.list({ worldviewId: collectionId })).length })
  const { data: boundGroups = 0 } = useQuery({ queryKey: ['groups', 'bound-world', collectionId], queryFn: async () => (await api.groups.list()).filter((group) => group.worldviewId === collectionId).length })
  ...
  async function save() {
    ...
    await api.worldbookEntries.put({ ...editing, title: editing.title.trim(), content: editing.content.trim(), keywords: [...new Set(keywords.split(/[、,，\n]+/).map((value) => value.trim()).filter(Boolean))], updatedAt: Date.now() })
    await api.worldbookCollections.patch(collectionId, { updatedAt: Date.now() })
    invalidate('worldbookEntries', 'worldbookCollections')
    setEditing(null)
  }
  ...
  async function importSelected() {
    ...
    try {
      ...
      await api.worldbookEntries.bulkPut(results.map((result, index) => ({ ...result.entry, sourceOrder: entries.length + index })))
      const usedAt = Date.now()
      for (const item of selected) await api.libraryItems.patch(item.id, { lastUsedAt: usedAt })
      await api.worldbookCollections.patch(collectionId, { updatedAt: Date.now() })
      invalidate('worldbookEntries', 'libraryItems', 'worldbookCollections')
  ...
  async function deleteWorld() {
    ...
    const rows = await api.worldbookEntries.list({ collectionId })
    if (rows.length) await api.worldbookEntries.bulkDelete(rows.map((entry) => entry.id))
    await api.worldbookCollections.delete(collectionId)
    invalidate('worldbookEntries', 'worldbookCollections')
  ...
  return <div className="relative flex h-[var(--app-height)] flex-col overflow-hidden bg-[#f4f4f6]">
    ...
    <div className="flex-1 overflow-y-auto px-4 pb-8">
      <section className="mt-3 rounded-xl bg-white p-4"><div className="flex justify-between gap-3"><div><p className="font-medium text-gray-900">{entries.length} 个正史条目</p><p className="mt-1 text-xs text-gray-400">{boundContacts} 个联系人 · {boundGroups} 个群聊属于此世界</p></div><button type="button" onClick={async () => { const name = window.prompt('世界观名称', collection.name)?.trim(); if (name) { await api.worldbookCollections.patch(collection.id, { name, updatedAt: Date.now() }); invalidate('worldbookCollections') } }} className="text-xs text-blue-600">重命名</button></div><div className="mt-3 grid grid-cols-3 gap-2 text-center text-[10px]"><div className="rounded-lg bg-gray-50 p-2 text-gray-600">总内容<br/><b>{formatEstimatedTokens(estimateWorldbookTokens(entries))}</b></div><div className="rounded-lg bg-amber-50 p-2 text-amber-700">每轮常驻<br/><b>{formatEstimatedTokens(estimateWorldbookTokens(permanent))}</b></div><div className="rounded-lg bg-blue-50 p-2 text-blue-600">按需触发<br/><b>{formatEstimatedTokens(estimateWorldbookTokens(triggered))}</b></div></div>{estimateWorldbookTokens(permanent) > 4000 && <p className="mt-2 rounded-lg bg-amber-50 px-3 py-2 text-xs text-amber-700">常驻内容超过4000 Token，可能明显挤占聊天记录和人物记忆。</p>}</section>

...
export function WorldbookCollectionPage() {
  ...
  return <div className="relative flex h-[var(--app-height)] flex-col overflow-hidden bg-[#f4f4f6]">
    ...
    <div className="flex-1 overflow-y-auto px-4 pb-8">
      <section className="mt-3 rounded-xl bg-white p-4">
        ...
        <div className="mt-3 grid grid-cols-3 gap-2 text-center">
          ...
          <div className="rounded-lg bg-[var(--ui-special-soft)] px-2 py-2"><p className="text-[10px] text-[var(--ui-special-ink)]">底层世界观</p><p className="mt-0.5 text-xs font-medium text-[var(--ui-special-ink)]">{formatEstimatedTokens(foundationalTokens)}</p></div>
        </div>
        ...
        {foundational.length > 0 && <p className={`mt-2 rounded-lg px-3 py-2 text-xs leading-relaxed ${foundationalTokens > 4000 ? 'bg-amber-50 text-amber-700' : 'bg-[var(--ui-special-soft)] text-[var(--ui-special-ink)]'}`}>底层世界观 {foundational.length} 条，{formatEstimatedTokens(foundationalTokens)}。{foundationalTokens > 4000 ? '内容较长，可能明显占用模型上下文。' : '会作为全局最高优先级正史发送。'}</p>}
      </section>
    ...
    {editing && <div className="absolute inset-0 z-30 flex items-center bg-black/30 p-5"><div className="max-h-[92%] w-full overflow-y-auto rounded-2xl bg-white p-4">
      ...
      <label className="mt-2 flex items-start gap-3 rounded-lg bg-[var(--ui-special-soft)] px-3 py-3"><input type="checkbox" checked={editing.foundationalWorldview === true} onChange={(event) => setEditing({ ...editing, foundationalWorldview: event.target.checked })}/><span><span className="block text-sm font-medium text-[var(--ui-special-ink)]">底层世界观</span><span className="mt-0.5 block text-[11px] leading-relaxed text-[var(--ui-special-ink)]">启用后默认传给所有主要内容生成，并作为全局最高优先级正史。</span></span></label>

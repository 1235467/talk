  ...
  return (
    <div className="flex h-[var(--app-height)] flex-col overflow-hidden bg-[#f4f4f6]">
      ...
      <div className="mt-3 flex-1 overflow-y-auto bg-white px-4 py-4">
        ...
        {settings.experienceMode === 'free' && <label className="mb-4 block text-xs font-medium text-gray-400">使用哪份提示词预设？<select value={settings.defaultPresetName ?? ''} onChange={(event) => settings.setSettings({ defaultPresetName: event.target.value || undefined })} className="mt-2 w-full rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm text-gray-800"><option value="">出厂默认</option>{promptPresets.filter((preset) => !preset.isFactory).map((preset) => <option key={preset.name} value={preset.name}>{preset.name}</option>)}</select><span className="mt-1 block text-[11px] font-normal leading-relaxed text-gray-400">联系人按名字引用预设；以后改预设内容会一起生效。</span></label>}

...
export function SaveLoadPage() {
  ...
  async function loadSlot(slot: number) { if (!window.confirm('回档会覆盖当前所有业务数据和设置，确定继续吗？')) return; setBusy(slot); try { await loadSaveSlot(slot); window.location.reload() } catch (e) { setMessage(String(e)); setBusy(null) } }

...
export function WorldbookEntrySelector({ open, selectedIds, onChange, onClose }: Props) {
  ...
  return <div className="absolute inset-0 z-40 flex flex-col bg-[#f4f4f6]">
    <div className="flex h-12 shrink-0 items-center justify-between border-b border-gray-100 bg-white px-4"><button type="button" onClick={onClose} className="text-sm text-gray-500">取消</button><h2 className="font-medium text-gray-900">选择额外世界观</h2><button type="button" onClick={onClose} className="text-sm font-medium text-[var(--ui-special-ink)]">完成</button></div>
    <div className="flex-1 overflow-y-auto px-4 pb-6">
      ...
      <div className="space-y-3">{collections.map((collection) => {
        ...
        return <section key={collection.id} className="overflow-hidden rounded-xl bg-white">
          ...
          {isExpanded && <div className="border-t border-gray-100">{collectionEntries.map((entry) => <label key={entry.id} className="flex items-start gap-3 border-b border-gray-50 px-4 py-3 last:border-0"><input type="checkbox" checked={selected.has(entry.id)} onChange={() => toggle(entry.id)} className="mt-1"/><span className="min-w-0 flex-1"><span className="block text-sm text-gray-800">{entry.title}</span><span className="mt-1 line-clamp-2 block text-xs leading-relaxed text-gray-400">{entry.content}</span><span className="mt-1 flex flex-wrap gap-1 text-[10px]"><span className="rounded bg-gray-100 px-1.5 py-0.5 text-gray-500">{formatEstimatedTokens(estimateWorldbookTokens([entry]))}</span>{!entry.enabled && <span className="rounded bg-gray-100 px-1.5 py-0.5 text-gray-500">当前停用</span>}{entry.foundationalWorldview && <span className="rounded bg-[var(--ui-special-soft)] px-1.5 py-0.5 text-[var(--ui-special-ink)]">底层世界观</span>}{entry.keywords.length === 0 && <span className="rounded bg-amber-50 px-1.5 py-0.5 text-amber-600">常驻</span>}</span></span></label>)}</div>}
        </section>

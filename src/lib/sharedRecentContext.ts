...
export const SHARED_CONTEXT_MAX_CHARS = 3_000
...
export async function recentSharedOriginalContext(
  ...
  const relevantGroupIds = new Set(
    groups
      .filter((group) => group.memberContactIds.some((contactId) => ids.has(contactId)))
      .map((group) => group.id),
  )
  const conversationIds = new Set(
    conversations
      .filter((conversation) => (
        ...
        && (
          (conversation.contactId != null && ids.has(conversation.contactId))
          || (conversation.groupId != null && relevantGroupIds.has(conversation.groupId))
        )
      ))
  ...
  const relevantMomentIds = new Set(
    moments
      .filter((moment) => ids.has(moment.contactId))
      .map((moment) => moment.id),
  )
  for (const comment of momentComments) {
    if (ids.has(comment.authorContactId)) relevantMomentIds.add(comment.momentId)
  }
  const relevantMoments = moments.filter((moment) => relevantMomentIds.has(moment.id))
  for (const moment of relevantMoments.slice(-maxMoments)) {
  ...
  const momentById = new Map(relevantMoments.map((moment) => [moment.id, moment]))
  for (const comment of momentComments
    .filter((comment) => relevantMomentIds.has(comment.momentId))
  ...
  const contextHeader = `【近期跨场景原文时间线】\n以下是近期真实记录，越靠后越新。用它维持睡觉、出门、情绪和话题等短期状态；后面的明确状态会覆盖前面的状态。状态会随现实时间自然失效，例如昨晚说“睡了”不代表第二天仍在睡。不得把未发生的事补成事实。其他场景中的内容只用于维持世界状态，角色不能表现得像亲耳听过，也不得在群聊或朋友圈公开泄露。\n【状态承接规则】用户已经明确说出新状态时，把它当作已确认事实并直接承接当前消息；不要为了展示记忆而把它重复成“你不睡了？”“你起来了？”“你要喝咖啡？”之类的确认问句。最新消息是邀请、问题或请求时，先直接回应它。只有记录彼此真正矛盾、用户措辞不确定，或缺少完成当前回应所必需的信息时才追问。`
  if (contextHeader.length >= maxChars) return contextHeader.slice(0, maxChars)

  const bodyMaxChars = maxChars - contextHeader.length - 1
  const ordered = rows.sort((a, b) => a.at - b.at).map((row) => row.text)
  const kept: string[] = []
  let chars = 0
  for (let i = ordered.length - 1; i >= 0; i--) {
    const separatorChars = kept.length > 0 ? 1 : 0
    const remaining = bodyMaxChars - chars - separatorChars
    if (remaining <= 0) break
    if (ordered[i].length > remaining) {
      if (kept.length === 0) kept.push(ordered[i].slice(0, remaining))
      break
    }
    kept.push(ordered[i])
    chars += ordered[i].length + separatorChars
  }

...
- 联系人名片的 Beta 模式是真正的 IndexedDB 数据库分支：开启时复制正式 `talk-db` 到独立数据库并重载切换，虚拟时间只在用户发消息时触发补全；关闭时删除整个分支并回到从未修改的正式库。不要改回“写正式库后逐表恢复”的快照方案。Beta 开启时暂停全局后台自动任务，防止真实时间写入冻结分支。
